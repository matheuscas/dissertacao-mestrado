...
\textbf{Aqui ainda vai faltar a parte de falar das medidas de avaliação estatística e das novas medidas a serem usadas nos resultados. Depois da seção seguinte, essa aqui vai ser mudada bastante.}

A avaliação dos resultados do sistema de mineração de opinião desse trabalho foi feita usando validação cruzada de 10 faces. As medidas para avaliar o desempenho da classificação da classificação do sistema foram a \textit{accuracy}, \textit{recall}, \textit{precision} e a pontuação F1. A \textit{accuracy} \ref{eq:acccuracy} é uma medida da razão entre a soma dos documentos positivos (TP) e negativos (TN) corretamente classificados corretamente e o total de documentos classificados, sejam eles corretos ou não.

\begin{equation}
Accuracy = (\sum TP + \sum TN) / Total
\label{eq:acccuracy}
\end{equation}

A \textit{recall} mede a razão entre os documentos classificados corretamente numa categoria e o total de documentos que realmente pertencem àquela categoria. O \textit{recall} mais utilizado é o positivo, também conhecido como sensibilidade da classificação, que avalia a razão entre a quantidade de documentos positivos classificados corretamente (TP) e esse mesmo número somado à quantidade de falsos negativos (documentos positivos que foram classificados como negativos - TN) \ref{eq:recall}. 
...
A \textit{precision}, por outro lado, mede a razão entre os documentos classificados corretamente numa categoria e os documentos classificados nessa mesma categoria. A \textit{precision} mais comum é a positiva que mede a razão entre TP e a soma deste com a quantidade de documentos negativos que foram classificados como positivos (falsos positivos - FP) \ref{eq:precision}. 

\begin{equation}
Precision = TP / (\sum TP + \sum FP)
\label{eq:precision}
\end{equation}

E a medida F1 considera a \textit{precisão} e a \textit{recall} \ref{eq:f1}. Essa medida é normalmente considerada como a média ponderada entre a \textit{precisão} e a \textit{recall} \ref{eq:f1} \ref{chaovalit2005movie};
...
A tabela \ref{table:medidas} ilustra a região em que cada medida atua.

\begin{table}[!h]
	\centering
    \begin{tabular}{llll}
    População & Positivos & Negativos & -- \\ \hline
    Classificado como positivo & TP & FP & \textbf{Precision} (\ref{eq:precision}) \\
    Classificado como negativo  & TP & FN& -- \\
    \textbf{Accuracy} (\ref{eq:acccuracy}) & \textbf{Recall} (\ref{eq:recall})  & -- & -- \\
    \end{tabular}
    \caption{Medidas utilizadas na avaliação do sistema fuzzy de mineração de opinião.}
	\label{table:medidas}
\end{table}