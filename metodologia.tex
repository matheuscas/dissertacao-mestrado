...
\todo[inline]{Neste primeiro parágrafo, o termo abordagem está sendo usado também para classificar o tipo do nosso problema, que é um problema de classificação. Recomendo nao fazer isso.}
\todo[inline]{Alterei}
...
\todo[inline]{Esse descarte de documentos com 3 de score vai ser motivo de questionamentos}
...
A tokenização dos documentos divide o conteúdo de cada documento em sentenças e, por sua vez, em palavras para que o marcador gramatical (ou \textit{tagger}) possa identificar as classes gramaticais das palavras do documento. O marcador gramatical usado foi o proposto por \citeonline{brill1995transformation}, e também usado em trabalhos relacionados a esta pesquisa \cite{chaovalit2005movie, taboada2008extracting, taboada2011lexicon}. O marcador gramatical é um sistema que processa um texto num determinado idioma, identifica e atribui rótulos para cada palavra nesse texto, como substantivos, verbos, adjetivos, advérbios, dentre outros \footnote{A lista completa dos rótulos para o idioma inglês pode ser encontrado em: \url{https://www.ling.upenn.edu/courses/Fall_2003/ling001/penn_treebank_pos.html}}. 

\todo[inline]{Eu substitui "partes de discurso" por "rótulos". }
\todo[inline]{Ok}
...
Neste processo,também extraímos tipos especiais de bigrams e trigrams: n-grams negados (e.g. \textit{not bad}, \textit{nothing special}). Nós aplicamos uma versão simplificada da técnica usada por \citeonline{das2001yahoo} para detecção de negação. N-grams negados podem tanto inverter a polaridade local de um termo ou sentimento geral de uma frase ou documento, quanto podem intensificar a polaridade geral (e.g. \textit{not only good but amazing}). Além disso, \citeonline{taboada2008extracting} demonstraram que, embora pequeno, o tratamento de negação em mineração de opinião, na média, produz melhores resultados na classificação dos documentos. 
...
\todo[inline]{Este último parágrafo não pode ser: Ao fim do estágio de pré-processamento, cada documento é transformado num vetor de n-grams, o qual é passado para a etapa de transformação.}
\todo[inline]{matheus: ja estava alterado. Voce quem mudou?}
...
Após a delimitação dos valores das características de acordo com a regra do três-sigma, definimos um único formato de conjunto fuzzy para todas as variáveis do SBRF. O formato escolhido foi o triangular, por sua simplicidade, e também, pelo fato de ser muito utilizado na literatura \cite{alcala2009multiobjective, gacto2010integration, antonelli2012multi, cardenas2012multiobjective}.\todo[inline]{Matheus, a forma como aparece a citação está estranha. O trabalho de Cárdenas e Camargo, há apenas dois autores, mas aparece a palavra et al. E no trabalho de Gacto, Alcalá, Herrera, deveria aparecer apenas o nome do primeiro autor, seguido de et al.}
\todo[inline]{matheus cardoso: eu ajeitei o de Cardenas, pois tinha um "Others" lá, mas o et al de Gacto eu estou por fora, é o latex que está fazendo assim. Já verifiquei na referencia e só tem 3 autores mesmo.}
...
\todo[inline]{matheus: optei pela opcao 1. Veja se consegui, professor.}
...
\todo[inline]{Matheus, eu acho que neste pseudocodigo, vc deveria separar a eliminação das regras duplicadas, das regras conflitantes. Isto porque, se a regra está duplicada, não é necessário calcular grau, apenas se exclui uma delas}
\todo[inline]{matheus: dividi os algoritmos}
...
\todo[inline]{Acho bom inserir uma referencia para estas medidas. Poderia ser esse: García-Pedrajas and Pérez-Rodríguez (2012) Multi-selection of instances: A straightforward way to improve evolutionary instance selection, Applied Soft Computing 12 (2012) 3590–3602. 
Pois acho melhor que o Wikipedia :)}
\todo[inline]{matheus: inserido}
